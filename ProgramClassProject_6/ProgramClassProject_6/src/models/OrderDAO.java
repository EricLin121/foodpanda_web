...
public class OrderDAO {
    ...
    public boolean insertCart(Order cart) {
        ...
        String query = "insert into `sale_order`(order_num,total_price,"
                + "customer_name,customer_phone, customer_address,cutlery,contactless) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            state.setInt(6,cart.getCutlery());
            state.setInt(7,cart.getContactless());
            state.execute();
            success = true;
        } catch (SQLException ex) {
    ...
    public boolean insertOrderDetailItem(OrderDetail item) {
        ...
        try {
            ...
            state.setInt(3,item.getQuantity().getSelectionModel().getSelectedIndex()+1);
